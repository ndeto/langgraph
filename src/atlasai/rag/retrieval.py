...
def load_web_page(url: str) -> list[Document]:
    docs = UnstructuredLoader(web_url=url).load()
    return docs
...
llm = ChatOpenAI(api_key=SecretStr(api_key), temperature=1, model="gpt-5.4-mini")
...
def grade_documents(
    ...
    prompt_comp = GRADE_PROMPT | llm.bind_tools(
        [retriever_tool]
    )

    res: AIMessage = prompt_comp.invoke({"question": question, "context": context})
...
def rewrite_question(state: MessagesState):
    ...
    rewrite_res = rewrite_summarizer.invoke({"question": q})
...
def generate_answer(state: MessagesState):
    ...
    results = []
    while runs < 3:
        res = generate_summary.invoke({"question": question, "context": context})

        results.append(res.content)
        runs += 1

    res = generate_summary.invoke({"question": question, "context": results})
